...
function getStreamExtractor(fileName: string, password?: string) {
  const stream = fs.createReadStream(`./testFiles/${fileName}`);
  return unrar.createExtractorFromData({ data: stream, password });
}
...
describe('Data Test', () => {
  ...
  it('Stream Archive Comment', async () => {
    const extractor = await getStreamExtractor('WithComment.rar');
    const { arcHeader, fileHeaders } = extractor.getFileList();
    assert.deepStrictEqual(arcHeader, {
      comment:
        'Test Comments for rar files.\r\n\r\n测试一下中文注释。\r\n日本語のコメントもテストしていまし。',
  ...
  it('Stream Header encryption', async () => {
    const extractor = await getStreamExtractor('HeaderEnc1234.rar');
  ...
  it('Stream File encrypted with different passwords', async () => {
    const extractor = await getStreamExtractor('FileEncByName.rar');
    const { arcHeader, fileHeaders } = extractor.getFileList();
    assert.deepStrictEqual(arcHeader, {
      comment: '',
      flags: {
  ...
  it('Stream Header encryption with password', async () => {
    const extractor = await getStreamExtractor('HeaderEnc1234.rar', '1234');
    const { arcHeader, fileHeaders } = extractor.getFileList();
    assert.deepStrictEqual(arcHeader, {
      comment: 'Hello, world',
      flags: {
  ...
  it('Stream Extract Header encryption file', async () => {
    const extractor = await getStreamExtractor('HeaderEnc1234.rar', '1234');
    const { arcHeader, files } = extractor.extract();
    assert.deepStrictEqual(arcHeader, {
      comment: 'Hello, world',
  ...
  it('Stream Extract Files by callback', async () => {
    const extractor = await getStreamExtractor('FileEncByName.rar');
    const { files } = extractor.extract({ files: (fileHeader) => !fileHeader.flags.encrypted });
    const list = [...files];
  ...
  it('Stream Extract File encrypted with different passwords (no password)', async () => {
    const extractor = await getStreamExtractor('FileEncByName.rar');
    const { files } = extractor.extract();
  ...
  it('Stream Extract File encrypted with different passwords (multiple passwords)', async () => {
    const extractor = await getStreamExtractor('FileEncByName.rar', '1234');
    let { files } = extractor.extract({ files: ['2中文.txt', '1File.txt'], password: '2中文' });
  ...
  it('Stream Extract File with folders', async () => {
    const extractor = await getStreamExtractor('FolderTest.rar');
    const { files } = extractor.extract();
